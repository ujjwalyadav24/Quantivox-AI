...
export default function TechnicalIndicators() {
  ...
  useEffect(() => {
    ...
    async function loadIndicators(symbol: string) {
      setLoading(true);

      try {
        const response = await getTechnical(symbol);
        setData(response);
      } catch (error) {
        console.error(error);
        setData(null);
  ...
  const trend = data.ema20 > data.ema50 ? "Bullish" : "Bearish";

  const trendColor =
    trend === "Bullish" ? "text-green-400" : "text-red-400";
